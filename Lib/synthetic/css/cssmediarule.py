...
from . import cssrule, cssrulelist, cssstylerule, csscharsetrule, cssfontfacerule, cssimportrule, cssnamespacerule, marginrule

#import cssutils
import xml.dom
from ..stylesheets import medialist

# from syffutils.serialize import *

# newser = CSSSerializer()
...
class CSSMediaRule(cssrule.CSSRuleRules):
    ...
    def _getCssText(self):
        ...
        med_rule = newser.do_CSSMediaRule(self)
        return med_rule
    ...
    def _setCssText(self, cssText):  # noqa: C901
        ...
        if self._type(attoken) != self._prods.MEDIA_SYM:
            ...
            if '{' == self._tokenvalue(end) or self._prods.STRING == self._type(end):
                self.media = medialist.MediaList(parentRule=self)
            ...
            if '}' != self._tokenvalue(braceOrEOF):
                ...
                self.cssRules = cssrule.CSSRuleList()
                seq = []  # not used really
    ...
    def insertRule(self, rule, index=None):
        ...
        # check hierarchy
        if (
            isinstance(rule, csscharsetrule.CSSCharsetRule)
            or isinstance(rule, cssfontfacerule.CSSFontFaceRule)
            or isinstance(rule, cssimportrule.CSSImportRule)
            or isinstance(rule, cssnamespacerule.CSSNamespaceRule)
            or isinstance(rule, marginrule.MarginRule)
        ):
            self._log.error(
