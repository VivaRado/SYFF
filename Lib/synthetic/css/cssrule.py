...
import xml.dom
from ..util import Base2
from .cssrulelist import CSSRuleList
from . import cssrulelist
from . import cssrule
...
class CSSRule(Base2):
    ...
    """
    UNKNOWN_RULE = 0
    ":class:`syffutils.css.CSSUnknownRule` (not used in CSSOM anymore)"
    STYLE_RULE = 1
    ":class:`syffutils.css.CSSStyleRule`"
    CHARSET_RULE = 2
    ":class:`syffutils.css.CSSCharsetRule` (not used in CSSOM anymore)"
    IMPORT_RULE = 3
    ":class:`syffutils.css.CSSImportRule`"
    MEDIA_RULE = 4
    ":class:`syffutils.css.CSSMediaRule`"
    FONT_FACE_RULE = 5
    ":class:`syffutils.css.CSSFontFaceRule`"
    PAGE_RULE = 6
    ":class:`syffutils.css.CSSPageRule`"
    NAMESPACE_RULE = 10
    """:class:`syffutils.css.CSSNamespaceRule`,
    Value has changed in 0.9.7a3 due to a change in the CSSOM spec."""
    COMMENT = 1001  # was -1, cssutils only
    """:class:`syffutils.css.CSSComment` - not in the offical spec,
    Value has changed in 0.9.7a3"""
    VARIABLES_RULE = 1008
    """:class:`syffutils.css.CSSVariablesRule` - experimental rule
    not in the offical spec"""

    MARGIN_RULE = 1006
    """:class:`syffutils.css.MarginRule` - experimental rule
    not in the offical spec"""
...
class CSSRuleRules(CSSRule):
    ...
    def _prepareInsertRule(self, rule, index=None):
        ...
        elif isinstance(rule, cssrulelist.CSSRuleList):
            # insert all rules
            for i, r in enumerate(rule):
                self.insertRule(r, index + i)
            return True, True

        elif not isinstance(rule, cssrule.CSSRule):
