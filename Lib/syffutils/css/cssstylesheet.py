...
from ..util import _Namespaces, _readUrl
...
class CSSStyleSheet(StyleSheet):
    ...
    def _getCssText(self):
        ...
        return _ser.do_CSSStyleSheet(self)
    ...
    def _setCssText(self, cssText):  # noqa: C901
        ...
        def mediarule(expected, seq, token, tokenizer):
            # parse and consume tokens in any case
            rule = CSSMediaRule(parentStyleSheet=self)
            rule.cssText = self._tokensupto2(tokenizer, token)
            if rule.wellformed:
                self.insertRule(rule)
            return 3
        ...
        def ruleset(expected, seq, token, tokenizer):
            # parse and consume tokens in any case
            rule = CSSStyleRule(parentStyleSheet=self)
            rule.cssText = self._tokensupto2(tokenizer, token)
            if rule.wellformed:
                self.insertRule(rule)
            return 3
    ...
    def insertRule(self, rule, index=None, inOrder=False, _clean=True):  # noqa: C901
        ...
        elif isinstance(rule, CSSRuleList):
            # insert all rules
