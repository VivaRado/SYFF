...
from ..prodparser import PreDef, Prod, ProdParser, Sequence
from ..helper import normalize, pushtoken
from .mediaquery import MediaQuery
from ..util import _NewListBase
...
class MediaList(_NewListBase):
    ...
    def _getMediaText(self):
        return _ser.do_stylesheets_medialist(self)
    ...
    def _setMediaText(self, mediaText):  # noqa: C901
        ...
        if ok:
            mediaTypes = []
            finalseq = Seq(readonly=False)
            commentseqonly = Seq(readonly=False)
            for item in seq:
                # filter for doubles?
                if item.type == 'MediaQuery':
